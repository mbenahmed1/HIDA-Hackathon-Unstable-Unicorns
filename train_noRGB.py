...
def train(hyperparameters: argparse.Namespace):
    ...
    # set up the dataset
    drone_images = DroneImages(hyperparameters.root, return_RGB=False)
    train_data, test_data = torch.utils.data.random_split(drone_images, [0.8, 0.2])

    # initialize MaskRCNN model
    model = MaskRCNN(in_channels=2, image_mean=[0.5, 0.5], image_std=[0.225, 0.225])
    if load_weight:
        cpt = torch.read(weight_path)
